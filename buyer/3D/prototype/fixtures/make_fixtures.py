...
Writes `fixtures/models.json` and five models under `fixtures/models/`. Open
...
ways the page must not care about:
    ...
    fixture_side_entry_box -- the same box with its entry off the +X wall,
    declared `front: +x`. The page must open facing that entry, and draw the
    width along that side, with no code that knows this fixture (#117).
...
# The same bench off the +X wall, centred on it, for the side-entry fixture.
SIDE_BENCH = ("bench", (0.45, 0.33, 0.2),
              (W + 0.6, W + 1.0, 0.10, 0.55, -D / 2 - 0.7, -D / 2 + 0.7))

MANIFEST_BLOCKS = ("sets", "presence", "disclosure", "views", "dimensions")


def identity(model_id, name, note, front="+z", entry=None):
    ident = {
        "id": model_id,
        "name": name,
        "area_sf": 460,
...
BOXES = [
    ...
    {
        # THE SAME BOX, ENTERED FROM THE SIDE (#117). Its bench -- its entry --
        # stands off the +X wall, and the manifest says so. The page must open
        # on that side with no code knowing this fixture exists, and draw the
        # width along the front, which here runs along Z.
        "identity": identity(
            "fixture_side_entry_box", "Fixture: box entered from the side",
            "A test fixture whose front is +X, not +Z: the first view must "
            "face its entry, and the overlay must draw its width along that "
            "side (#117). Not a real model.",
            front="+x", entry="fixture_bench"),
        "parts": PARTS + [SIDE_BENCH],
        "dimensions": {
            "units": "feet",
            "main_body": {"width": 23.0, "depth": 20.0,
                          "note": "fixture: the wall box, width along the +X front"},
            "height_to_ridge": round(3.15 / FT, 3),
        },
    },
]
...
def render_model(spec, files):
    ...
    # A fixture that names its entry is held to its front exactly as a real
    # export is (#117), so the side-entry box cannot drift to another side.
    if ident.get("entry_node"):
        lod0 = (("lod0.glb", data) if "levels" not in spec
                else HERE / levels.get("lod0", ""))
        bad = model_contract.front_problems(lod0, ident["front"], ident["entry_node"])
        if bad:
            raise SystemExit(f"{model_id}: " + "; ".join(bad))
