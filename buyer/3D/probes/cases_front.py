...
CASES = [
    ...
    Case(G, "a manifest whose model block is a string",
         _set(manifest, "model", "barn"), index_gates(), "model block is a str, not an object"),
    Case(G, "a manifest whose model block is a list",
         _set(manifest, "model", ["barn"]), index_gates(), "model block is a list, not an object"),
    Case(G, "app.js swaps the +x and +z directions under the right names",
         replace(app_js, ("  '+x': [1, 0, 0],\n", "  '+x': [0, 0, 1],\n"),
                 ("  '+z': [0, 0, 1],\n", "  '+z': [1, 0, 0],\n")),
         page_gates(), "app.js points front '+x' along [0, 0, 1]"),
