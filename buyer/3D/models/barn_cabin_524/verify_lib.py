...
# inside_mesh() knows no building, so it lives in the kit (#126). It is
# imported here so every gate's `from verify_lib import inside_mesh` is
# unchanged. The known-answer cases below name this building's objects and
# coordinates, so they stay with it.
sys.path.append(str(Path(__file__).resolve().parents[2]))  # buyer/3D, for adu_kit
from adu_kit.verify_lib import inside_mesh  # noqa: E402,F401
