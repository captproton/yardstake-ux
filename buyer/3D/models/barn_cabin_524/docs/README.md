...
| **Tier 3** | Fixtures, appliances, furniture | **not started** — sketch only |

Merged to `main` in [#57](https://github.com/captproton/yardstake-ux/pull/57) and [#58](https://github.com/captproton/yardstake-ux/pull/58). The placement developer is unblocked —
`lod2` and their handoff are on `main`, and unchanged at **24.1 KB**.

**Tiers 1 and 2 are complete bar KTX2 compression**, which is optimisation
rather than necessity: `lod0` is 419 KB against a 4 MB ceiling. Tier 3 is the
only substantial work remaining.
...
| Work | Notes |
|---|---|
| **T3:** fixtures and furniture | The whole of what is left of substance. Sketch only; firm it up before building. **Start with licensing** — it is the long-lead item |
