...
7. [#131](https://github.com/captproton/yardstake-ux/issues/131) **Next.** P4 materials, the stucco/siding swap, three levels, manifest, baseline. **Laurel's front is `-z`**: Blender +Y exports to glTF −Z. Its spec declares `meta.index.front` with `glb: "-z"` and `entry: Leaf_D-1`, and the export refuses anything else. **Laurel lands on the page with no page code; closes [#111](https://github.com/captproton/yardstake-ux/issues/111).**
